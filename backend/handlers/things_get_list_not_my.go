...
func GetThingsListNotMy(w http.ResponseWriter, r *http.Request) {
	SetupCORS(&w)
	...
	case "lost":
		rows, err := DB.Query(
			"SELECT * FROM lost_thing WHERE NOT advertisement_owner=? ORDER BY name;",
			username,
		)
		if err != nil {
	...
	case "found":
		rows, err := DB.Query(
			"SELECT * FROM found_thing WHERE NOT advertisement_owner=? ORDER BY name;",
			username,
		)
		if err != nil {

...
func UserLogin(w http.ResponseWriter, r *http.Request) {
	...
	// TODO: maybe refactor and make two separate variables (username, password
	// hash) instead of one struct userAccountData
	err := row.Scan(
...
func UserGetEmail(w http.ResponseWriter, r *http.Request) {
	// Handler to get email of own user from the JWT access
	SetupCORS(&w)
	...
	case err == sql.ErrNoRows:
		...
		return
	case err != nil:
		msg := "Error getting user email: " + err.Error()
		Logger.Error(msg)
		http.Error(w, msg, http.StatusInternalServerError)
		return
	default:
		...
		w.Write(jsonData)
		Logger.Info("Success. Received own user email")
		return
	}
}

func UserOtherGetEmail(w http.ResponseWriter, r *http.Request) {
	// Handler to get email of the other user by username
	SetupCORS(&w)
	...
	username :=
		r.FormValue("username")
	if username == "" {
		msg := "error: the \"username\" parameter is required"
		Logger.Error(msg)
		http.Error(w, msg, http.StatusBadRequest)
		return
	}
	...
	default:
		...
		w.Write(jsonData)
		Logger.Info("Success. Received other user email")
		return
	}
}

func UserGetUsername(w http.ResponseWriter, r *http.Request) {
	SetupCORS(&w)
	...
	w.Write(jsonData)
	Logger.Info("Success. Received username")
}

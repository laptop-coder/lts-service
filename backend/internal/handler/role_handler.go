...
func (h *RoleHandler) AssignPermissions(w http.ResponseWriter, r *http.Request) {
	...
	if len(permissionIDsFields) == 0 {
		...
		helpers.ErrorResponse(h.log, w, "the list of permissions cannot be empty", http.StatusBadRequest)
		return
	}
	...
	for i, s := range permissionIDsFields {
		...
		if err != nil {
			...
			helpers.ErrorResponse(h.log, w, "cannot convert IDs of permissions from string to uint64", http.StatusInternalServerError)
			return
		}

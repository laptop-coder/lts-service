...
+ Суперадминистратор (единственное право --- генерировать инвайт-коды для создания аккаунтов администраторов). Аккаунт создаётся отдельно и может быть только один. Как `root` в `linux`
+ Администратор сервиса (модерация объявлений, например)
+ Администрация школы (директор, заместитель директора)
+ Сотрудники школы (повар, охранник, уборщик, дворник)
+ Учителя
+ Родители
+ Ученики
...
+ Восстановить пароль от аккаунта суперадминистратора невозможно. Если его забыть, придётся вручную удалять аккаунт из БД и затем создавать новый через сайт
...
+ `superadmin` --- логин и хеш пароля единственного аккаунта суперадминистратора
+ `user` --- адреса электронной почты и хеши паролей всех пользователей, кроме суперадминистратора
+ `role` --- роли пользователей (администратор сервиса, администрация школы, сотрудник и т.д.)
...
+ `school_administrator` --- данные представителей администрации школы, связь с `user` один к одному
+ `staff` --- данные сотрудников школы, связь с `user` один к одному
+ `room` --- аудитории, учебные кабинеты
+ `students_group` --- классы/группы учеников/студентов (`1А`, `1А гр. 1`, `гр. 1234`)
+ `subject` --- преподаваемые предметы
+ `teacher` --- данные учителей, связь с `user`, `room` и `students_group` --- один-к-одному

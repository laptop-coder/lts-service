    ...
    await axiosInstance
      ...
      .then((response) => {
        if (response.status == 200) {
          window.location.replace(HOME__ROUTE);
        }
      })
      .catch((error) => {
        alert(
  ...
  return (
    <Form onsubmit={handleSubmit}>
      ...
      <AuthFormSubmitButton
        ...
        name='login_user_form_submit_button'
      />
      <AuthFormOtherChoice>
        Нет аккаунта? <A href={REGISTER_USER__ROUTE}>Регистрация</A>
      </AuthFormOtherChoice>
    </Form>
  );
};

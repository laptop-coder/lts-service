...
const StudentGroupsManagement = () => {
  ...
  const loadStudentGroups = async () => {
    try {
      const data = await api.get<{ studentGroups: StudentGroup[] }>("/student_groups");
      setStudentGroups(data.studentGroups);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Ошибка загрузки учебных групп (классов)");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createStudentGroup = async (e: Event) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Ошибка создания учебной группы (класса)");
    } finally {
      setCreating(false);
    }
  };
  ...
  const deleteStudentGroup = async (id: number) => {
    if (!confirm("Удалить учебную группу (класс)? Это действие нельзя отменить.")) return;
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Ошибка удаления учебной группы (класса)");
    } finally {
      setDeletingId(null);
    }
  };
  ...
  return (
    <div class="space-y-6">
      ...
      <Show when={hasPermission(PERMISSIONS.STUDENT_GROUP_CREATE)}>
        <form onSubmit={createStudentGroup} class="bg-gray-50 p-4 rounded-lg space-y-3">
              ...
              <For each={studentGroups()}>
                {(studentGroup) => (
                  <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 text-sm text-gray-500">{studentGroup.id}</td>
                    <td class="px-4 py-3 font-medium">{studentGroup.name}</td>
                    <td class="px-4 py-3 text-right">
                      <Show when={hasPermission(PERMISSIONS.STUDENT_GROUP_DELETE)}>
                        <button
                          onClick={() => deleteStudentGroup(studentGroup.id)}
                          disabled={deletingId() === studentGroup.id}
                          class="text-red-600 hover:text-red-800 disabled:opacity-50"
                        >
                          {deletingId() === studentGroup.id ? "Удаление..." : "Удалить"}
                        </button>
                      </Show>
                    </td>
                  </tr>
